...
- **5列**: `更新日時, 建屋, 装置ID, チャンバーID, ステータス`（1行目はヘッダ）
- **文字コード**: UTF-8 / UTF-8(BOM) / Shift_JIS(cp932) を自動判定
- **ヘッダ照合**: 1行目の列名を内部列名へ変換してから照合します。DBの列名そのものに一致する必要はありません。

| 1行目のヘッダ | 取込 |
|---|---|
| `更新日時,建屋,装置ID,チャンバーID,ステータス`（仕様の日本語） | ✅ |
| `updated_at,building,equipment_id,chamber_id,status`（英語=内部列名） | ✅ |
| 前後にスペースがある（` 更新日時 ` 等） | ✅（自動でトリム） |
| 列の順序が違う / 余分な列がある | ✅（名前で対応・余分列は無視） |
| 表記ゆれ（`装置ID`→`装置番号`、`equipment_id`→`tool_id` 等） | ❌ 下記マッピングで対応 |
| 途中にスペース・大文字小文字違い（`装置 ID`, `装置Id`） | ❌ 別名扱い |

必須5列が揃わないと取込はエラーになります（不足列・検出列をメッセージに表示）:
```
ValueError: CSVに必要な列がありません: ['equipment_id'] / 検出された列: [..., '装置番号', ...]
```
`scheduler.py` 経由ならその回の取込だけスキップして継続（スケジューラは停止しない）、
手動取込・アプリ初回生成では例外がそのまま上がります。**表記ゆれは次の設定で吸収できます。**
